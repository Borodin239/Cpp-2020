...
_start:
                ...
                sub             rsp, 2 * 128 * 8 		; rsp - stack-pointer, указывает на место, где справа всё полезное и писать нельзя, а слева пусто
                lea             rdi, [rsp + 128 * 8] 		; вычисляет эффективный адресс источника - обязательно память и помещает в приёмник (обязатиельно регистр) 
                mov             rcx, 128 			; размер long с которым мы работаем
                call            read_long 			; вызов функции чтения первого лонга
                mov             rdi, rsp			; я так понял что rdi - первый символ нашего числа (первого)
                call            read_long 			; вызов функции чтения второго лонга
                lea             rsi, [rsp + 128 * 8] 		; смотри выше
	
		mov		rax, rsi			; меняю rsi и rdi местами, ибо в rdi - второй операнд сейчас
		mov		rsi, rdi
		mov		rdi, rax

                call            sub_long_long 			; вызов функции сложения
...
sub_long_long:
                ...
                clc						; сброс флага переноса CF (0+255 значение 0 и флаг переноса 1)
